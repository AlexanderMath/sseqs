...
def msa(queries, 
    ...
    if not blocking: 
        ...
        def _target():
            with th.cuda.device(gpu): msa(
                queries, filenames, num_msas, save_raw, 
                verbose, bs, fft_rank, top_fft, top_sw,       
                top_sw_affine, return_timings, sync_time, 
                dlength, gpu, blocking=True)
